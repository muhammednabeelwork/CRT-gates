...
export default function ControllerDashboard({ navigate }: { navigate: (path: string) => void }) {
  ...
  // Anonymous sign‑in for server‑side uploads
  useEffect(() => {
    (async () => {
      try {
  ...
  return (
    <div className="relative h-screen w-screen bg-black text-white select-none"
      style={{
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center space-y-4">
        ...
        <button onClick={() => (window.location.href = BACK_URL)}
          className="mt-4 px-3 py-2 bg-[#066D55]/10 border border-[#066D55] rounded text-[#96f1c9] mx-auto block">
          Back
        </button>
      </div>
